...
const signer = {
  ...
  sign: async (payload: Uint8Array) => new Uint8Array(await crypto.subtle.sign("HMAC", await crypto.subtle.importKey("raw", key, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]), payload)),
};
const verifier = {
  verify: async ({ payload, signature }: { payload: Uint8Array; signature: Uint8Array }) =>
    crypto.subtle.verify("HMAC", await crypto.subtle.importKey("raw", key, { name: "HMAC", hash: "SHA-256" }, false, ["verify"]), signature, payload),
};
const document: AgentDiscoveryDocument = {
  ...
  publisher: { id: "https://example.com", name: "Example", jwksUri: "https://example.com/.well-known/jwks.json" },
  capabilities: [{ id: "travel.search", title: "Search travel", description: "Search flights and hotels", tags: ["travel", "flights"], effects: ["read"] }],
  interfaces: [
    { type: "a2a", url: "https://agents.example/a2a", protocolVersion: "1.0" },
    { type: "mcp", url: "https://agents.example/mcp", protocolVersion: "2025-11-25" },
  ],
  authentication: { schemes: ["oauth2"], protectedResourceMetadata: "https://agents.example/.well-known/oauth-protected-resource" },
  categories: ["travel"], tags: ["booking"], languages: ["en"], paymentProtocols: ["ap2"],
...
describe("agent discovery", () => {
  test("signs, verifies, and detects tampering", async () => {
    ...
    const tampered = { ...signed, document: { ...signed.document, name: "Impostor" } };
  ...
  test("projects A2A and serves cacheable discovery", async () => {
    ...
    const response = await createAgentDiscoveryHandler({ documents: [signed] })(new Request(`https://agents.example${ABSOLUTE_AGENT_PATH}`));
  ...
  test("ranks verified capability matches", async () => {
    const signed = await signAgentDocument(document, signer);
    const registry = createAgentRegistry({ store: createMemoryAgentRegistry(), verifier });
    await registry.publish(signed);
    const result = await registry.search({ text: "travel flights", verifiedOnly: true });
  ...
  test("exposes searchable registry metadata and denies anonymous publishing by default", async () => {
    ...
    const result = await handler(new Request("https://registry.example/v1/agents?q=flights"));
    expect(result.status).toBe(200);
    expect((await result.json()).results[0].agent.id).toBe(document.id);
    const denied = await handler(new Request("https://registry.example/v1/agents", { method: "POST", body: JSON.stringify(signed) }));
